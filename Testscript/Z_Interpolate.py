...
def interpolateQuantity(file_list, quantity="T", z_wall_spec="no_wall", z_stoich="0.111", n_x=200, n_y=50):
    ...
    quantity_all_runs = np.array(quantity_all_runs)
    chi_st_all_runs = np.array(chi_st_all_runs)
    ...
    ax.set_title(f"z_wall {z_wall_spec}")
    for i, chi_st in enumerate(chi_st_sorted):
        ax.scatter(0.111, chi_st, label=f"chi_stoich{chi_st_sorted[i]}")
        ax.legend()
    #plt.show()
    # Save as csv

interpolateQuantity(file_list, z_wall_spec=0.4, n_x=400, n_y=400, quantity="T")
interpolateQuantity(file_list, z_wall_spec=0.4, n_x=400, n_y=400, quantity="h_mass")
plt.show()

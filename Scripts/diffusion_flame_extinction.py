...
f.P = 1.e5  # 1 bar
f.fuel_inlet.mdot = 0.1  # kg/m^2/s
f.fuel_inlet.X = 'H2:1'
...
#f.solve(loglevel=1,refine_grid=False, auto=True)
print("Solve without wall done.")
...
refine_grid = True
f.set_refine_criteria(ratio=3.0, slope=0.1, curve=0.05, prune=0.04)
f.set_initial_guess("Scripts/Data/enthalpy.h5", group="initial") 

#f.set_refine_criteria(ratio=3.0, slope=0.5, curve=0.5, prune=0.03, enthahlpy=True, enthalpy_curve=0.05)
#runtime, factor_last_working = ut.solve_with_wall(f, wall_params, 
#                          delta_T_max=1.0, loglevel=1, factor=1000,
#                          refine_grid=refine_grid, auto=False, factor_increase=2.0)
runtime, factor_last_working = ut.solve_with_wall(f, wall_params,
                                factor_last_working=1000,
                                delta_T_max=1.0, loglevel=0, refine_grid=refine_grid, auto=True)


print(f"wall_params[factor]: {wall_params["factor"]}")
file_name = "Scripts/Data/new_try_No5.h5"

wall_params["factor"] = factor_last_working
z_stoich_val = ut.get_z_stoich(gas, wall_params, reaction_mechanism)        
entry = "iniitial"
all_entries = [entry]
ut.save_with_attributes(f, file_name, entry, wall_params, z_stoich_val, info=True, runtime=runtime)
...
while True:
    ...
    if not np.isclose(np.max(f.T), temperature_limit_extinction):
        ...
        n_last_burning = n
        entry = f"extinction/{n:04d}"
        wall_params["factor"] = factor_last_working
        ut.save_with_attributes(f, file_name, entry, wall_params, 
                                z_stoich_val, runtime=runtime, info=True)
        ...
        entry = f"extinction/{n_last_burning:04d}"
...
plt.savefig(file_name/ "figure_T_max_a_max.png")
plt.show()

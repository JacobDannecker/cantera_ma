...
for i, file in enumerate(file_list):
    ...
    if list(h5_file.keys())[0] == "extinction":                                 
        ...
        if len(extinct) > 1:                                                    
            # Only plot the extict flame that is closest to the last burning flame
            strains = {}                                                        
            for n in extinct:                                                   
    ...
    for  name in names:                                                              
        ...
        # Plot T
        ax1[0].plot(f.mixture_fraction("H"), f.T)                       
        # Plot h
        ax1[1].plot(f.mixture_fraction("H"), f.h)                       
        # Plot Y
        ax2[0].plot(f.mixture_fraction("H"), f.Y[idx_H2])              
        ax2[1].plot(f.mixture_fraction("H"), f.Y[idx_O2])              
        ax2[2].plot(f.mixture_fraction("H"), f.Y[idx_H2O])              
        ax2[3].plot(f.mixture_fraction("H"), f.Y[idx_O])              
        ax2[4].plot(f.mixture_fraction("H"), f.Y[idx_H])              
